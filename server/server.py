...
def dev_exec(req: dict):
    ...
    The code runs in this module's globals; set `result` to return a
    value."""
    import contextlib
    import io as _io
    buf = _io.StringIO()
    ...
    try:
        ...
        return {"ok": True, "stdout": buf.getvalue()[-4000:],
                "result": repr(ns.get("result"))[:2000]}
    except Exception as e:
        import traceback as _tb
        return {"ok": False, "stdout": buf.getvalue()[-2000:],
...
def dev_vram():
    """Name every resident module holding GPU memory (leak forensics)."""
    import gc as _gc
    out = {"torch_allocated_GB": round(
    ...
    for obj in _gc.get_objects():
        try:
            if isinstance(obj, torch.nn.Module):
                ...
                if b > 0:
                    k = type(obj).__name__
                    seen[k] = seen.get(k, 0) + b
        except Exception:
            continue
    out["modules_GB"] = {k: round(v / 1e9, 2) for k, v in
                         sorted(seen.items(), key=lambda x: -x[1])[:12]
                         if v > 0.05e9}
    return out
...
def dev_reload_fluxfm():
    """Reload flux_fmwdm_interactive and rebind the cached sampler."""
    global _warm_fluxfm
    if "_warm_fluxfm" not in globals():
        _warm_fluxfm = None
    import importlib
    import flux_fmwdm_interactive as _fm
    importlib.reload(_fm)
    swapped = False
    if _warm_fluxfm is not None:
        _new = _fm.FluxFMWDMInteractive.__new__(_fm.FluxFMWDMInteractive)
        _new.__dict__ = _warm_fluxfm.__dict__
        _warm_fluxfm = _new
        swapped = True
    print(f"[dev] fluxfm module reloaded (sampler swapped={swapped})",
          flush=True)
...
def list_models():
    ...
    for key, (label, repo, env_var) in _MODEL_REPOS.items():
        out.append({
            ...
            "available": ((_model_loadable(key) and (key != "fluxfm"
                               or os.path.exists(_FLUXFM_LORA)))
                          or key == resident),
...
def run_generate(job_id: str, req: dict) -> None:
    # Wall-time anchor for the otherwise-untimestamped log stream.
    import time as _t0
    print(f"[gen-start] job {job_id} model={req.get('model')} "
          f"prompt={str(req.get('prompt'))[:60]!r} "
          f"(wall {_t0.strftime('%H:%M:%S')})", flush=True)
    ...
    # right after finish_round errored, because the previous worker still
    # holds the lock for its final decode/cluster teardown (>6s). Wait
    # honestly with a visible stage instead of failing the artist's click.
    jobs[job_id].update({"stage": "Waiting for the previous round to wrap up...",
                         "progress": 2})
    if not _model_lock.acquire(timeout=90):
        jobs[job_id].update({
            "status": "error", "result": None,
            "error": "Another session is still generating \u2014 wait for it to finish or cancel it, then try again.",
            "progress": 0, "stage": "Busy",
        })
        print(f"[worker-lock] job {job_id} gave up after 90s "
              f"(wall {__import__('time').strftime('%H:%M:%S')})", flush=True)
        return
    ...
    try:
        ...
        import traceback
        import time as _t
        # Loud, timestamped death marker: the log's lines are otherwise
        # untimestamped, which turned a stale tqdm line into a phantom
        print(f"[worker-died] job {job_id}: {type(exc).__name__}: {exc} "
              f"(wall {_t.strftime('%H:%M:%S')})", flush=True)
        traceback.print_exc()
        try:
            import torch
            if torch.cuda.is_available():
...
_warm_krea2 = None      # Krea-2 sampler kept resident across generations (warm reuse)
...
def _free_previous_sampler():
    ...
    """
    global _active_sampler, _warm_flux, _warm_krea2
...
def _run_span_round1(job_id, req, sampler, output_dir, session_id, device,
    ...
    job = jobs[job_id]
    prompt = req["prompt"]
    # too). Krea-2's hardware path is serial regardless, so batch=1 makes each
    # image solve start-to-finish and pop — display mirrors execution at zero
    # cost. FLUX keeps real batches (serializing it costs true 3-4x).
    _sb_default = 1 if str(req.get("model", "")).lower() == "krea2" else 8
    batch_size = max(1, min(int(req.get("span_batch_size", _sb_default)
                                or _sb_default), 16))
        ...
        def _augment_bg():
            try:
                ...
                for stage_n in stages:
                    ...
                    # appeared for him — reruns of the same prompt hit the
                    # variation cache, fresh=[] skipped this, and the payload
                    # stayed empty. Cached readings are still THIS round's
                    # readings; the artist must see them either way.)
...
def _do_generate(job_id: str, req: dict) -> dict:
    ...
    # Recorded on the job for the round-ETA estimator (submit_selection).
    jobs[job_id]["_n_particles"] = num_particles
    jobs[job_id]["_model"] = str(req.get("model", "krea2") or "krea2")
    ...
    if model == "flux":
        ...
        K = len(variations)
        # the earlier fix patched a different augment site that this path
        # never executes; verify_live's same-prompt-twice check proved it).
        # This is the site the krea2 span/interactive path actually runs, so
        # record here, unconditionally, cached or fresh.
        jobs[job_id]["_span_prompts"] = list(variations)
        ...
        # WARM REUSE (mirrors FLUX): keep the resident Krea-2 sampler across
        # jobs so every round no longer pays the ~20s reload. Validate the
        # object is intact (a prior CREATE MAP / other backend may have gutted
        # it via _free_previous_sampler) AND the text encoder is present
        # (needed for set_prompts diversity). Otherwise reconstruct fresh.
        global _warm_krea2
        warm_ok = (_warm_krea2 is not None
                   and getattr(_warm_krea2, "transformer", None) is not None
                   and getattr(getattr(_warm_krea2, "pipeline", None),
                               "text_encoder", None) is not None)
        if warm_ok:
            sampler = _warm_krea2
            set_progress(8, "Reusing warm KREA-2 model...")
        else:
            _free_previous_sampler()   # also drops any warm FLUX
            # keep_text_encoder=True ALWAYS: the encoder stays resident for
            # re-prompting across jobs (Krea-2 init: "warm mode - VAE/
            # transformer still fit"). We no longer free it per job.
            sampler = _load_sampler_with_progress(
                job_id, "Loading the KREA-2 image model (one-time, ~2 min)...",
                "krea2",
                lambda: Krea2Interactive(prompt=prompt, device=device,
                                         dtype=torch.bfloat16,
                                         keep_text_encoder=True))
            _warm_krea2 = sampler
        _active_sampler = sampler
        ...
        if _krea_augment:
            ...
            try:
                ...
                # WARM MODE: keep the Gemma text encoder resident across jobs so
                # the next round re-embeds variations without a ~20s reload
                # (Krea-2 init confirms VAE + transformer + encoder fit on 44GB).
                pass
        K = max(1, len(variations))
        # READINGS RECORD — at the site the krea2 branch ACTUALLY executes
        # (third attempt; the first two patched the span-function and generic
        # branches, which krea2 never runs — it has no sample_span_round and
        # falls back to this classic flow). Unconditional: cached or fresh.
        jobs[job_id]["_span_prompts"] = list(variations)
        # SESSION RECIPE at the same proven-executing site (the span-path
        # copy never runs for krea2 — verified absent on sessions
        # 7355e3af/3256713c). Records the effective config so any session
        # is exactly replayable.
        try:
            import json as _json
            import datetime as _dt
            with open(os.path.join(output_dir, "session_recipe.json"), "w") as _rf:
                _json.dump({
                    "prompt": prompt, "model": "krea2",
                    "master_seed": req.get("master_seed"),
                    "readings_K": K, "variations": variations,
                    "n_images": req.get("n_images"),
                    "lookahead_steps": req.get("lookahead_steps"),
                    "rho": req.get("rho"), "order": req.get("order"),
                    "recorded_at": _dt.datetime.now().isoformat(timespec="seconds"),
                }, _rf, indent=1)
        except Exception as _re:
            print(f"[recipe] classic-path write failed: {_re}")
        ...
        if "_warm_fluxfm" not in globals():
            _warm_fluxfm = None
        _reuse_ffm = _warm_fluxfm is not None
        if not _reuse_ffm:
            _free_previous_sampler()
        # encoders after init, so every prompt the tree may assign must be
        # pre-encoded NOW via additional_prompts. Children draw pool
        # variations of the artist's prompt at spawn (semantic divergence
        # on top of latent renoise). Augment failures degrade to an empty
        # pool = tree silently off, never a broken load.
        _ptree_pool = []
        if req.get("augment", True):
            try:
                set_progress(4, "Exploring different ways to read your prompt...")
                import prompt_augment
                # CPU always: Qwen-on-GPU beside FLUX + the encoder hop
                _aug_dev = "cpu"
                # Caller-supplied readings skip the local LLM entirely
                _explicit = [x for x in (req.get("prompt_variations") or [])
                             if isinstance(x, str) and x.strip()]
                if _explicit:
                    print(f"[ptree] {len(_explicit)} caller-supplied "
                          f"readings (LLM skipped)", flush=True)
                _pool_wild = (list(_explicit)[:8] if _explicit else
                              [v for v in prompt_augment.augment_prompt(
                                  prompt, n=6, device=_aug_dev)
                               if v and v.strip()][:6])
                _pool_restyle = (list(_explicit)[:8] if _explicit else
                                 [v for v in prompt_augment.augment_prompt(
                                     prompt, n=6, device=_aug_dev,
                                     mode="restyle")
                                  if v and v.strip()][:6])
                _ptree_pool = _pool_wild + _pool_restyle
                jobs[job_id]["_span_prompts"] = [prompt] + list(_pool_wild)
            except Exception as _pt_exc:
                print(f"[ptree] pool augment failed: {_pt_exc}")
        if _reuse_ffm:
            sampler = _warm_fluxfm
            try:
                set_progress(8, "Reading your prompt (warm model)...")
                torch.cuda.empty_cache()
                for _a in ("text_encoder", "text_encoder_2"):
                    _te = getattr(sampler.pipeline, _a, None)
                    if _te is not None:
                        _te.to(device)
                sampler.prompt = prompt
                _pe0, _ppe0 = sampler._encode_prompt_internal(prompt)
                sampler.cached_prompt_embeds = _pe0
                sampler.cached_pooled_prompt_embeds = _ppe0
                for _p in (_ptree_pool or []):
                    if _p not in sampler.prompt_cache:
                        _e, _pp = sampler._encode_prompt_internal(_p)
                        sampler.prompt_cache[_p] = (_e.cpu(), _pp.cpu())
            finally:
                for _a in ("text_encoder", "text_encoder_2"):
                    _te = getattr(sampler.pipeline, _a, None)
                    if _te is not None:
            ...
            sampler = _load_sampler_with_progress(
            job_id, "Loading the FluxFM image model (one-time, ~1 min)...",
            "fluxfm", lambda: FluxFMWDMInteractive(
                prompt=prompt, device=device,
                additional_prompts=_ptree_pool,
                # Offline prod: the vendored fork can't fetch hub metadata —
                # load from the staged local snapshot like the flux branch.
                model_id=(os.environ.get("FLUX_LOCAL_PATH")
        ...
        # deep sigma, medium/palette words at mid sigma): stage-1 deep
        # spawns draw WILD readings, later shallow re-anchors draw RESTYLE.
        sampler._ptree_pool_wild = list(
            _pool_wild if req.get("augment", True) else [])
        sampler._ptree_pool_restyle = list(
            _pool_restyle if req.get("augment", True) else [])
        # Session recipe parity with the krea2 classic path.
        try:
            import json as _json
            import datetime as _dt
            with open(os.path.join(output_dir, "session_recipe.json"),
                      "w") as _rf:
                _json.dump({
                    "prompt": prompt, "model": "fluxfm",
                    "master_seed": req.get("master_seed"),
                    "clone_mode": req.get("clone_mode"),
                    "ptree_pool": _ptree_pool,
                    "n_images": req.get("n_images"),
                    "rho": req.get("rho"),
    ...
    if req.get("span_round1") and hasattr(sampler, "sample_span_round"):
        ...
        # config per session so quality comparisons replay evidence instead
        # of inferring it (the 6738947f-vs-midnight comparison was blocked
        # exactly here: no recorded seed or readings count).
        try:
            import json as _json
            import datetime as _dt
            _recipe = {
                "prompt": req.get("prompt"), "model": req.get("model"),
                "master_seed": _session_master_seed,
                "augment": bool(req.get("augment", True)),
                "num_variations_req": req.get("num_variations"),
                "n_images": req.get("n_images"),
                "lookahead_steps": req.get("lookahead_steps"),
                "lookahead_steps_first": req.get("lookahead_steps_first"),
                "rho": req.get("rho"), "order": req.get("order"),
                "num_steps": req.get("num_steps"),
                "span_batch_size": req.get("span_batch_size"),
                "recorded_at": _dt.datetime.now().isoformat(timespec="seconds"),
            }
            with open(os.path.join(output_dir, "session_recipe.json"), "w") as _rf:
                _json.dump(_recipe, _rf, indent=1)
        except Exception as _re:
            print(f"[recipe] write failed: {_re}")
    ...
    def mock_input(prompt_text):
        ...
        selection_event.clear()
        # Close the ETA loop: measure the round that just finished and fold
        # its per-child rate into the per-model EMA the next estimate uses.
        _bt0 = jobs[job_id].pop("_branch_t0", None)
        if _bt0 is not None:
            _elapsed = time.time() - _bt0
            _nc = max(1, int(jobs[job_id].get("_n_particles") or 6))
            _mk = str(jobs[job_id].get("_model") or "krea2")
            _per_child = max(0.2, (_elapsed - _ROUND_OVERHEAD_S) / _nc)
            _old = _ROUND_RATE_EMA.get(_mk)
            _ROUND_RATE_EMA[_mk] = (_per_child if _old is None
                                    else 0.7 * _old + 0.3 * _per_child)
            jobs[job_id]["eta_s"] = None
    ...
    # PROGRESSIVE FIRST-TILE PAINT: emit each round-1 preview to the canvas the
    # moment it's decoded+saved (tiles land ~4-8s apart) instead of blocking on
    # the whole interval to finish + cluster (~25-30s). Partials get a transient
    # golden-spiral coord; the interval's real clustered layout overwrites
    # job["images"] when it completes. Signature-gated (only Krea-2 accepts it).
    import math as _math_pp
    def _on_preview(interval_idx, p_idx, path):
        try:
            # ALL intervals stream now, not just round 1. The old
            # `interval_idx != 1: return` silently dropped every breed-round
            # preview, so the frontend's mid-breed progressive paint (which
            # watches job["images"] grow) never had anything to consume.
            j = jobs.get(job_id)
            if j is None or j.get("_cancelled"):
                return
            url = f"/images/{session_id}/interval_{int(interval_idx)}/{Path(path).name}"
            # New interval starting to stream -> drop the previous round's
            # tiles from the payload. Without this reset a breed preview would
            # APPEND to the old round's 6 (length 7 > round size) and the
            # frontend's partial-paint guard would correctly refuse it.
            if int(j.get("interval") or 0) != int(interval_idx):
                imgs = []
            else:
                imgs = list(j.get("images") or [])
            if any(im.get("url") == url for im in imgs):
                return
            ang = p_idx * 2.399963267948966
            r = min(0.44, 0.10 + 0.055 * _math_pp.sqrt(max(int(p_idx), 0)))
            x = min(0.95, max(0.05, 0.5 + r * _math_pp.cos(ang)))
            y = min(0.95, max(0.05, 0.5 + r * _math_pp.sin(ang)))
            imgs.append({"id": len(imgs), "url": url, "x": x, "y": y,
                         "preview": True})
            j["images"] = imgs
            j["interval"] = int(interval_idx)
            j["stage"] = f"painting your spread — {len(imgs)} so far…"
        except Exception:
            pass
    if "on_preview" in inspect.signature(sampler.sample_interactive).parameters:
        sample_kwargs["on_preview"] = _on_preview
    # LIVE STEP PREVIEWS: during the solve the sampler emits a ~free RGB
    # projection of every particle's current denoised estimate (fixed per-
    # particle URLs, overwritten each emission; "seq" is the cache-buster the
    # frontend appends as a query param). Signature-gated like on_preview.
    def _on_step_preview(k, num_steps, paths, active=None):
        try:
            j = jobs.get(job_id)
            if j is None or j.get("_cancelled"):
                return
            j["step_previews"] = {
                "step": int(k), "num_steps": int(num_steps),
                "seq": int(j.get("step_previews", {}).get("seq", 0)) + 1,
                "urls": [f"/images/{session_id}/steps/{Path(p).name}"
                         for p in paths],
                # Which tile the GPU is actively refining (null = all
                # advancing together during the coupled segment solve).
                "active": (None if active is None else int(active)),
            }
        except Exception:
            pass
    if "on_step_preview" in inspect.signature(sampler.sample_interactive).parameters:
        sample_kwargs["on_step_preview"] = _on_step_preview
    # finish and previews immediately, instead of all advancing in lockstep.
    # Display mirrors execution. Per-request; frontend controls the default.
    if "seq_order" in inspect.signature(sampler.sample_interactive).parameters:
        sample_kwargs["seq_order"] = (
            str(req.get("order", "")).lower() == "sequential")
    ...
    try:
        # no_grad seals the ~19GB autograd-retention leak (fmwdm's main
        # loop runs bare; 43.6GB resident measured at warm-encode,
        with torch.no_grad():
...
def start_generate(req: GenerateRequest):
    ...
    # A blank prompt burned a full GPU round in the overnight matrix
    # (edge-empty: accepted -> generated 6 images of nothing-in-particular).
    # The UI disables GENERATE on empty input; the API now agrees.
    if not (req.prompt or "").strip():
        raise HTTPException(status_code=422,
                            detail="Give me a prompt first — even one word.")
...
def get_job(job_id: str):
    ...
    return {
        ...
        "step_previews": job.get("step_previews"),
        "eta_s": job.get("eta_s"),
        # The LLM's prompt readings (index 0 = the artist's exact prompt).
        # Stored since the span-augment work but never exposed; the artist
        # should be able to SEE how their prompt was interpreted.
        "prompt_variations": job.get("_span_prompts"),
...
# Self-calibrating breed-round pace: per-model EMA of measured seconds per
# child on THIS process (updated each time a round presents; seeds from the
# the estimate within a couple of rounds, no config anywhere.
_ROUND_RATE_EMA: dict = {}
_ROUND_OVERHEAD_S = 6.0
...
def submit_selection(job_id: str, req: SelectRequest):
    ...
    # ROUND ETA: predicted from THIS process's own measured pace (EMA of
    # per-child seconds by model), so GPU type, node contention, and model
    # differences are absorbed automatically instead of hardcoded. Breed time
    # scales with the CHILD COUNT (the session's spread), not with how many
    # ~5.5 s/child + ~6 s overhead (serial), flux ~1.3 s/child (batched).
    _model = str(job.get("_model") or "krea2")
    _n_children = int(job.get("_n_particles") or 6)
    _rate = _ROUND_RATE_EMA.get(_model, 5.5 if _model == "krea2" else 1.3)
    job["eta_s"] = int(round(_ROUND_OVERHEAD_S + _n_children * _rate))
    job["_branch_t0"] = time.time()
