...
class FluxDevInteractive:
            ...
            with torch.no_grad():
                for k in range(num_steps):
                    ...
                    v = self.predict_velocity(z, t_cur, guidance_scale,
                                              prompt_idx=p_idx_t)
                    z = z + (t_next - t_cur) * v
                    # PIXEL-SPACE DINO REPULSION (flag-gated, default off;
                    # Jerry 2026-08-26: "the dino repulsion loss should be in
                    # pixel space, not latent space"). Endpoint estimate
                    # x0_hat = z - t*v, decoded by the DIFFERENTIABLE tiny-VAE
                    # (taef1, ms-scale) into real pixels, embedded by DINO;
                    # reward = mean pairwise embedding distance; the gradient
                    # flows through DINO+taef1 back to z (identity pullback,
                    # sigma-annealed so late steps commit undisturbed).
                    # Whether this yields APPEARANCE diversity without
                    # fidelity loss is the OPEN question its A/B decides.
                    _fs = float(os.environ.get("FLUX_FMRG_S", "0") or 0)
                    if _fs > 0 and bsz > 1 and t_next > 0.35:
                        try:
                            with torch.enable_grad():
                                _x0 = (z - t_next * v).detach().float()
                                _x0.requires_grad_(True)
                                _px = self._taef1_decode_diff(_x0)  # [B,3,H,W] 0..1
                                _pv = torch.nn.functional.interpolate(
                                    _px, size=(224, 224), mode="bilinear",
                                    align_corners=False)
                                _pv = (_pv - _pv.new_tensor(
                                    [0.485, 0.456, 0.406]).view(1, 3, 1, 1)) \
                                    / _pv.new_tensor(
                                    [0.229, 0.224, 0.225]).view(1, 3, 1, 1)
                                _emb = self.dino_model(
                                    pixel_values=_pv.to(self.dtype)
                                ).pooler_output.float()
                                _d = torch.cdist(_emb, _emb)
                                _r = _d.sum() / max(bsz * (bsz - 1), 1)
                                _r.backward()
                            _g = _x0.grad
                            # FMRG-J: u_hat=(u/||u||)*||v||, lambda_t=eta*dt
                            # (velocity-magnitude rescale -> eta is reward-
                            # scale invariant; paper eta=1.0 at FLUX scale).
                            _gn = _g.flatten(1).norm(dim=1).clamp_min(1e-8)
                            _vn = v.flatten(1).norm(dim=1).clamp_min(1e-8)
                            z = z + ((_g / _gn.view(-1, 1, 1, 1))
                                     * _vn.view(-1, 1, 1, 1)
                                     * _fs * abs(t_next - t_cur)).to(z.dtype)
                        except Exception as _fe:
                            print(f"[fmrg-px] skipped k={k}: {_fe}")
    ...
    def _taef1_decode_diff(self, z: torch.Tensor) -> torch.Tensor:
        """DIFFERENTIABLE ms-scale decode via taef1 (tiny FLUX VAE) for the
        pixel-space repulsion reward. Lazy-loads from the offline hub (the
        weights are staged at boot). ASSUMPTION (per taesd-family usage):
        taef1 takes raw scheduler latents, no scaling_factor/shift — a
        mismatch would surface as degraded decodes and a failed A/B, which
        the harness montage would catch."""
        if getattr(self, "_taef1", None) is None:
            from diffusers import AutoencoderTiny
            self._taef1 = AutoencoderTiny.from_pretrained(
                "madebyollin/taef1", torch_dtype=torch.float32
            ).to(self.device).eval()
            for _p in self._taef1.parameters():
                _p.requires_grad_(False)
        out = self._taef1.decode(z.float()).sample
