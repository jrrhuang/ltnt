"""
FLUX FlowMap Interactive Sampler (512-res LoRA)

Interactive particle pruning using FLUX FlowMap with GLASS Flows for
stochastic clone divergence.

All ODE steps use flow map velocity (dual timestep t_cur → t_next),
NOT instantaneous velocity. This means predict_vector is called with
t_next = next timestep in the schedule (not t_cur).

Lookahead uses flow map steps marching along the pipeline schedule to t=0.
...
# Respect an explicitly-set HF_HOME (e.g. the /data cache in test jobs) —
# the unconditional scratch override stranded fresh nodes with empty caches.
if os.path.exists("/scratch/jerryhua") and "HF_HOME" not in os.environ:
...
def compute_linear_diversity_schedule(num_steps: int, num_resampling_steps: int) -> list:
    ...
    Places all resampling points in the first ~40% of steps with power-law spacing.
    """
...
class FluxTTInteractive(FluxFlowMapVLMSampler):
    ...
    def sample_interactive(self,
        ...
        # Resampling points
        if linear_diversity and num_resampling_steps > 0:
        ...
        for k in pbar:
            ...
            with torch.no_grad():
                # Anchors: flow map ODE step (t_next = next timestep, NOT t_cur)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_steps", type=int, default=8,
                        help="Total flow map steps (default 8: 4 segments × M=2)")
    ...
    parser.add_argument("--lora_path", type=str,
                        ...
                        help="Path to FlowMap LoRA weights (default: 512-res LoRA)")
    ...
    print(f"\nStarting interactive FMTT with GLASS Flows (ρ={args.rho}, linear_diversity={args.linear_diversity})...")
    best_image = sampler.sample_interactive(
        ...
        linear_diversity=args.linear_diversity,
