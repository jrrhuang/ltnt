...
import { editImageNanoBanana } from '../api_nanobanana'

// Shim: adapt the Nano Banana client to the { edited_b64 } shape the rest
// of this file expects. Gemini has no mask API, so when the user draws a
// region we tell the server to crop to it — the model only sees the
// selected area, and the compositor pastes the result back at those
// coordinates. Lasso is reduced to its bounding box for the crop.
async function editRegion(imageSrc, options) {
  const { prompt = '', region = null } = options || {}
  let sendRegion = region
  if (!sendRegion && Array.isArray(options?.normalizedRegions)) {
    sendRegion = options?.normalizedBbox || null
  }
  const result = await editImageNanoBanana(imageSrc, prompt, {
    n: 1,
    region: sendRegion,
  })
...
const getEffects = async () => []  // Nano Banana has no named effect list
...
export default function EditViewNanoBanana({ images = [], onBack, tourMode = false, onTourModeChange = () => {} }) {
  ...
  const tour = useTour('ltnt.tour.edit.nanobanana', { autoOpen: tourMode })
  ...
  async function handleApply() {
    ...
    try {
      ...
      // Region edits: Nano Banana returned only the edited CROP. Commit it
      // as a region layer so compositeLayerStack pastes it back into the
      // user's rect while leaving the rest of the image untouched.
      // Full-image edits (vary, or edit with no selection): returned image
      // is the full result — replace the original.
      if (mode === 'edit' && region) {
        const newLayer = {
            ...
            {currentSrc ? (
              ...
              >
                ...
                {/* Pending edit preview: the edited crop is stretched into
                    ...
                    for full-image edits). Matches compositeLayerStack. */}
                {pendingLayer && (() => {
