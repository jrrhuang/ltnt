...
import { compositeLayerStack } from '../api'
import { editImageOpenAI } from '../api_openai'

// Shim: adapt the OpenAI client to the { edited_b64 } shape the rest of
// this file expects from the Reve editRegion helper. Drops Reve-only
// options (effects, upscale_factor, operation) and maps regions/lassos
// into the OpenAI mask format.
async function editRegion(imageSrc, options) {
  const { operation = 'edit', prompt = '', region = null } = options || {}
  // Convert selection into OpenAI mask shapes. region is a rect layerExtra
  // when selectionType === 'rect'; the fuller layerExtra (with
  // normalizedRegions) also lives in options for lasso.
  const shapes = []
  if (options?.selectionType === 'rect' && region) {
    shapes.push({ type: 'rect', region })
  } else if (options?.selectionType === 'lasso' && Array.isArray(options.normalizedRegions)) {
    shapes.push({ type: 'lasso', polygons: options.normalizedRegions.map(r => r.points || []) })
  } else if (region) {
    shapes.push({ type: 'rect', region })
  }
  // n=1 for edit; Vary uses n=3 and splits into three results (handled in
  // the caller via three separate editRegion calls, so we keep n=1 here).
  const result = await editImageOpenAI(imageSrc, prompt, {
    shapes: shapes.length > 0 ? shapes : null,
    n: 1,
  })
  const [b64] = result.images_b64 || []
  if (!b64) throw new Error('OpenAI returned no image')
  return { edited_b64: b64, credits_remaining: null }
}
const getEffects = async () => []  // OpenAI has no named effect list
...
const EDIT_TOUR_STEPS = [
  {
    ...
    body: "Almost there! Here you can fine-tune one image at a time — describe a change, draw a box or lasso to target a region, or ask for small variations. Every step shows up in History on the right so you can jump back.",
    placement: 'center',
  },
  {
    target: '[data-tour="image-grid"]',
    title: 'Your picks',
    body: "These are the images you chose. Click any of them to switch — each one keeps its own history.",
    placement: 'left',
  },
  {
    target: '[data-tour="operations"]',
    title: 'What would you like to do?',
    body: "Describe a change ('make the hat red'), or ask for a few variations of the same image.",
    placement: 'right',
  },
  {
...
const OPERATIONS = [
  // OpenAI edits are full-image replacements driven by a prompt + optional
  // mask. UPSCALE / REMOVE_BG / EFFECTS are expressed via prompt instead.
  { id: 'edit',      label: 'EDIT'         },
...
export default function EditViewOpenAI({ images = [], onBack, tourMode = false, onTourModeChange = () => {} }) {
  ...
  const tour = useTour('ltnt.tour.edit.openai', { autoOpen: tourMode })
  ...
  async function handleApply() {
    ...
    // Edit requires a prompt; selection is optional (no selection = whole
    // image edit; with selection = mask-clipped region).
    ...
    try {
      ...
      // OpenAI edits are full-image replacements — we push a clean new
      // "original" entry rather than stacking layers, so /HISTORY reads as
      // a linear timeline.
      const newSrc      = `data:image/png;base64,${result.edited_b64}`
      const newOriginal = { id: genId(), type: 'original', src: newSrc, visible: true }
      await pushEntry([newOriginal], imgIdx)
      if (mode === 'edit') {
        setSelection(null)
        setPrompt('')
      }
  ...
  // ── Derived ───────────────────────────────────────────────────────────────
  // Edit only needs a prompt — selection is an optional mask. If the user
  // draws a region the edit is clipped to that area; if not, OpenAI edits
  // the whole image.
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', background: '#e8e8e8' }}>
          ...
          <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
            {currentSrc ? (
              ...
              >
                ...
                {/* Pending effect preview — CSS opacity blend on top of base.
                    For region/lasso edits, Reve returns a FULL-size edited
                    image, so we clip the overlay to the user's selection
                    shape. That way the preview matches what'll be committed
                    (rest of image stays original; only selection shows edit). */}
                {pendingLayer && (() => {
                  let clipPath
                  if (pendingLayer.selectionType === 'rect' && pendingLayer.region) {
                    const r = pendingLayer.region
                    const top    = r.y * 100
                    const right  = (1 - (r.x + r.w)) * 100
                    const bottom = (1 - (r.y + r.h)) * 100
                    const left   = r.x * 100
                    clipPath = `inset(${top}% ${right}% ${bottom}% ${left}%)`
                  } else if (pendingLayer.selectionType === 'lasso' &&
                             Array.isArray(pendingLayer.normalizedRegions) &&
                             pendingLayer.normalizedRegions[0]?.points?.length >= 3) {
                    const pts = pendingLayer.normalizedRegions[0].points
                    clipPath = `polygon(${pts.map(p => `${p.x * 100}% ${p.y * 100}%`).join(', ')})`
                  }
                  return (
                    <img
      ...
      <Tour
        ...
        storageKey="ltnt.tour.edit.openai"
      />
    </div>
  )
}
