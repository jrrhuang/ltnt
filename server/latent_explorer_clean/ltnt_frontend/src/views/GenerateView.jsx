...
const GENERATE_TOUR_STEPS = [
  ...
  },
  {
    ...
    body: "Click here to create your first batch. In a minute or two you'll see a handful of images to pick from.",
    placement: 'top',
  },
...
export default function GenerateView({ onImages, onExplore, onCreateMap, onBoard = () => {}, tourMode = false, onTourModeChange = () => {} }) {
  ...
  const [cloneMode, setCloneMode] = useState('glass')
  const [nImages, setNImages] = useState(6)
  ...
  const DIVERSITY_PRESETS = [
    { key: 'balanced', label: 'BALANCED', spawn_mode: 'glass',  rho: 0.4  },
    { key: 'wild',     label: 'WILD',     spawn_mode: 'sdedit', rho: 0.25 },
    { key: 'tight',    label: 'TIGHT',    spawn_mode: 'glass',  rho: 0.6  },
  ]
  ...
  async function handleGenerate() {
    ...
    const params = {
      ...
      rho: diversity.rho,
      spawn_mode: diversity.spawn_mode,
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg)' }}>
      ...
      <footer style={{
        ...
        <button
          onClick={() => setDiversityIdx(i => (i + 1) % DIVERSITY_PRESETS.length)}
          disabled={loading}
          title="Steer how different the generated variations are from each other"
          style={{
            color: 'rgba(255,255,255,0.55)',
            background: 'transparent',
            border: '1px solid rgba(255,255,255,0.18)',
