...
FLUX.1-dev, SD3.5, and Krea-2-Raw ship today; this doc is the contract for adding another.
...
> non-path-consistent flow** (FLUX-schnell/klein, SDXL-Turbo, Krea-Turbo, …) breaks the
> trajectory consistency → clones go off-manifold. Use the **base/dev** flow, not the turbo/distilled one.

## Required interface

A backend is a class `MyModelInteractive` (see `flux_fmtt_dev.py`, `sd35_fmtt.py`, `krea2_fmtt.py`).
It MUST implement:
...
`_do_generate` branches on `req["model"]` (`flux`/`sd35`/`krea2`) to construct the right class, then calls
